...
6. fetch_url(url: str, timeout: int = 30)
   - 功能：通过HTTP/HTTPS访问网页并返回网页内容
   - 参数：
     - url - 网页URL地址（字符串），必须以http://或https://开头
     - timeout - 请求超时时间（整数，默认30秒）
   - 返回：网页内容（内容过长时会截断至前5000字符）
...
- 访问网页：{"tool": "fetch_url", "args": {"url": "https://www.example.com"}}

## URL访问场景

当用户提供了URL地址（如 https://wttr.in/青城山、https://www.example.com 等）时，
你应该自动调用fetch_url工具来获取网页内容。

**正确示例：**
- 用户："访问 https://wttr.in/青城山" → 调用 {"tool": "fetch_url", "args": {"url": "https://wttr.in/青城山"}}
- 用户："帮我查看百度首页" → 调用 {"tool": "fetch_url", "args": {"url": "https://www.baidu.com"}}

**注意事项：**
- **URL参数中不要使用反引号包围，直接写URL**
- 直接使用用户提供的URL地址
- 如果用户输入的是中文URL，会自动进行URL编码
...
def execute_tool(tool_name: str, args: Dict[str, Any]) -> str:
    ...
    执行工具调用
    """
    from tools import list_directory, rename_file, delete_file, create_file, read_file, fetch_url
        ...
        while True:
            try:
                ...
                if url_match:
                    ...
                    original_url = original_url.rstrip('.,;:!?').rstrip('`').strip("'").strip('"')
                    user_input = f"请使用fetch_url工具访问以下URL并获取内容：{original_url}"
